...
public:
	vector <string> justify(vector <string> textIn) {
		int maxlen = 0 ;
		fori(textIn.size()) maxlen = ( maxlen < textIn[i].size()) ? textIn[i].size():maxlen;
		string buffer=""; fori(maxlen) buffer+=" ";
		fori(textIn.size()) {
			textIn[i] = buffer.substr(0,maxlen-textIn[i].size()) +  textIn[i];
		}
		return textIn;
	}
};
